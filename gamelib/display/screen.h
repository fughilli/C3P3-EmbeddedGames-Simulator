...
enum Color_t { BLACK = 0, WHITE = 1, INVERT = 2, NONE = 3 };
...
enum Font_justification_t { FJ_LEFT, FJ_RIGHT, FJ_CENTER };
...
struct Rect_t {
  ...
  Rect_t(screen_coord_t _x, screen_coord_t _y, screen_coord_t _w,
         screen_coord_t _h) {
    x = _x;
    y = _y;
    w = _w;
  ...
  Point_t upperLeft() { return Point_t(x, y); }
  Point_t lowerLeft() { return Point_t(x, y + h); }
  Point_t upperRight() { return Point_t(x + w, y); }
  Point_t lowerRight() { return Point_t(x + w, y + h); }

  Point_t center() { return Point_t(x + w / 2, y + h / 2); }

  void centerOn(const Point_t& cent) { operator+=(cent - center()); }
};
...
class Screen {
 ...
 public:
  ...
  void bitmap_nbx(screen_coord_t x, screen_coord_t y, const uint8_t* bitmap,
                  screen_coord_t w, screen_coord_t h, Bitmap_mode_t mode);
  void bitmap(const Bitmap_t* bmp, const Rect_t* srcRect, const Rect_t* dest,
              Bitmap_mode_t mode);
  ...
  void text(const Font_t& font, const Point_t& pt, const char* str,
            Bitmap_mode_t mode);
  void text_plus_offset(const Font_t& font, const Point_t& pt, const char* str,
