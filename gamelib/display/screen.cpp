...
Color_t Screen::getPixel(screen_coord_t x, screen_coord_t y) const {
  if (x < 0 || x >= m_width || y < 0 || y >= m_height) return NONE;
...
screen_coord_t Screen::getWidth() const { return m_width; }

screen_coord_t Screen::getHeight() const { return m_height; }
...
void Screen::bitmap_adj(const Bitmap_t* _bmp, const Rect_t* _srcRect,
  ...
  if (srcRect.w < 0) srcRect.w = bmp.w;
  if (srcRect.h < 0) srcRect.h = bmp.h;
...
void Screen::bitmap(const Bitmap_t* bmp, const Rect_t* srcRect,
  ...
  // Check bounds on destination point
  if (dest->x < 0 || dest->y < 0 || ((dest->x + srcRect->w) > m_width) ||
...
void Screen::bitmap_nbx(const Bitmap_t* bmp, const Rect_t* srcRect,
  ...
  // Generate the destination left and right side bitmasks (for preserving the
  // old contents around the bitmap)
  ...
  int32_t bmpRowOffset =
      bmpByteWidth *
      srcRect->y;  // Offset from bmp->data for the first byte in each row
  int32_t fbRowOffset =
      m_bytewidth *
      dest->y;  // Offset from framebuffer for the first byte in each row
  int32_t srcRectRowIndex, bmpColByteIndex;  // Indices; row, and byte in src

  int32_t bmpFirstByteRowOffset =
      srcRect->x /
      8;  // Offset from left edge of bmp to first byte of srcRect, in bytes
  int32_t fbFirstByteRowOffset =
      dest->x / 8;  // Offset from left edge of framebuffer to first byte of
                    // dest, in bytes

  int32_t numDestCols =
      (((dest->x + srcRect->w) % 8) ? ((dest->x + srcRect->w) / 8 + 1)
                                    : ((dest->x + srcRect->w) / 8)) -
      (dest->x / 8);

  uint32_t stagingArea;  // Place to store two consecutive bytes during copy
                         // operation (to solve spanning problem);
    ...
    for (bmpColByteIndex = 0; (bmpColByteIndex < numDestCols);
      ...
      switch (mode) {
        ...
        case MODE_OVERWRITE_INVERT:
          if (bmpColByteIndex == 0) {
            (*framebuffer)[destIndex] =
                ((*framebuffer)[destIndex] & dest_lEdgeMask) |
                (stagingArea & ~dest_lEdgeMask);
          } else if (bmpColByteIndex == numDestCols - 1) {
            (*framebuffer)[destIndex] =
                ((*framebuffer)[destIndex] & dest_rEdgeMask) |
        ...
        case MODE_BLEND_INVERT:
          if (bmpColByteIndex == 0) {
            (*framebuffer)[destIndex] =
                ((*framebuffer)[destIndex]) | (stagingArea & ~dest_lEdgeMask);
          } else if (bmpColByteIndex == numDestCols - 1) {
            (*framebuffer)[destIndex] =
                ((*framebuffer)[destIndex]) | (stagingArea & ~dest_rEdgeMask);
          } else {
            (*framebuffer)[destIndex] |= (stagingArea & 0xFF);
          }
          break;
        ...
        case MODE_MASK_INVERT:
          if (bmpColByteIndex == 0) {
            (*framebuffer)[destIndex] =
                ((*framebuffer)[destIndex]) &
                ((stagingArea & ~dest_lEdgeMask) | dest_lEdgeMask);
          } else if (bmpColByteIndex == numDestCols - 1) {
            (*framebuffer)[destIndex] =
                ((*framebuffer)[destIndex]) &
                ((stagingArea & ~dest_rEdgeMask) | dest_rEdgeMask);
          } else {
            (*framebuffer)[destIndex] &= (stagingArea & 0xFF);
          }
          break;
...
void Screen::textOption(const Font_t& font, const Point_t& pt, const char* str,
  ...
  if (fjust == FJ_RIGHT || fjust == FJ_CENTER) {
    if (fjust == FJ_RIGHT) drawpt += Point_t(maxw, 0);
    if (fjust == FJ_CENTER) drawpt += Point_t(maxw / 2, 0);
    ...
    do {
      ...
      if (fjust == FJ_RIGHT)
        screen.textlen(font, drawpt + Point_t(-linewidth, lineheight), lstart,
                       len, mode);
      if (fjust == FJ_CENTER)
        screen.textlen(font, drawpt + Point_t(-(linewidth / 2), lineheight),
                       lstart, len, mode);
...
void Screen::text(const Font_t& font, const Point_t& pt, const char* str,
                  Bitmap_mode_t mode) {
  textlen(font, pt, str, 0xFFFFFFFF, mode);
}

void Screen::textlen(const Font_t& font, const Point_t& pt, const char* str,
...
void Screen::text_plus_offset(const Font_t& font, const Point_t& pt,
                              const char* str,
                              Point_t& (*posmod)(uint32_t charnum),
                              Bitmap_mode_t mode) {
...
void Screen::hline_nbx(screen_coord_t line, screen_coord_t x1,
                       screen_coord_t x2, Color_t color) {
  ...
  if (x1 == x2)
    ...
    switch (color) {
      case WHITE:
        ...
        while (x1 < x2) (*framebuffer)[x1++] = 0xff;
        (*framebuffer)[x1] |= r_mask;
        break;
      case BLACK:
        (*framebuffer)[x1++] &= ~l_mask;
        while (x1 < x2) (*framebuffer)[x1++] = 0;
        (*framebuffer)[x1] &= ~r_mask;
        break;
      case INVERT:
        (*framebuffer)[x1++] ^= l_mask;
...
void Screen::hline(screen_coord_t line, screen_coord_t x1, screen_coord_t x2,
                   Color_t color) {
  if (x1 > x2) {
  ...
  if (x1 >= m_width || x2 < 0 || line < 0 || line >= m_height) return;

  hline_nbx(line, x1, x2, color);
}

void Screen::vline_nbx(screen_coord_t col, screen_coord_t y1, screen_coord_t y2,
                       Color_t color) {
...
void Screen::vline(screen_coord_t col, screen_coord_t y1, screen_coord_t y2,
                   Color_t color) {
  if (y1 > y2) {
  ...
  if (y1 < 0 || y2 > m_height || col < 0 || col >= m_width || color == NONE)
    return;
...
void Screen::box(const Rect_t* _box, Color_t color, Color_t fcolor) {
  if (_box != NULL) {
    box(_box->x, _box->y, _box->x + _box->w - 1, _box->y + _box->h - 1, color,
        fcolor);
  }
}

void Screen::circle_nbx(screen_coord_t x, screen_coord_t y, screen_coord_t r,
  ...
  if (fcolor != NONE) hline_nbx(y, x - r, x + r, fcolor);
...
void Screen::circle(screen_coord_t x, screen_coord_t y, screen_coord_t r,
                    Color_t color, Color_t fcolor) {
  int32_t f = 1 - r;
  int32_t ddF_x = 1;
  int32_t ddF_y = -2 * r;
  ...
  // there is a fill color
  if (fcolor != NONE) hline(y, x - r, x + r, fcolor);
...
void Screen::circle(const Point_t* pos, screen_coord_t r, Color_t color,
                    Color_t fcolor) {
  if (pos != NULL) {
...
#define BMP_PIX(__x, __y)                                                \
  (((bmp->data[(bmp_byte_width * (__y)) + ((__x) / 8)] << ((__x) % 8)) & \
    0x80) == 0x80)

void Screen::bitmap_scaled_nbx(const Bitmap_t* bmp, const Rect_t* srcRect,
  ...
  for (int32_t j = 0; j < destRect->h; j++) {
    for (int32_t i = 0; i < destRect->w; i++) {
      uint8_t pixel = BMP_PIX(srcRect->x + ((i * srcRect->w) / destRect->w),
                              srcRect->y + ((j * srcRect->h) / destRect->h));
...
void Screen::bitmap_scaled(const Bitmap_t* bmp, const Rect_t* srcRect,
                           const Rect_t* destRect, Bitmap_mode_t mode) {
  if (!bx(destRect)) return;
...
void Screen::triangleArea(const Point_t* p1, const Point_t* p2,
                          const Point_t* p3, Color_t fcolor) {
  int16_t wx4 = p1->x;
  int16_t wy4 = p1->y;
  int16_t wx5 = p1->x;
  int16_t wy5 = p1->y;
...
void Screen::triangle(const Point_t* _p1, const Point_t* _p2,
                      const Point_t* _p3, Color_t color, Color_t fcolor) {
  Point_t p1 = *_p1;
  Point_t p2 = *_p2;
  Point_t p3 = *_p3;
  ...
  if (fcolor != NONE) {
    ...
    if (p2.y == p3.y) {
      ...
      Point_t p4;
      p4.x = (screen_coord_t)((int32_t)p1.x +
                              (p2.y - p1.y) * (p3.x - p1.x) / (p3.y - p1.y));
      p4.y = p2.y;
